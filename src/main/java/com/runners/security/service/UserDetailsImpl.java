...
public class UserDetailsImpl implements UserDetailsService {
    ...
    private static List<SimpleGrantedAuthority> buildGrantedAuthorities(final Set<Role> roles){ // final yazmamızın nedeni setlenmeden kullanılmasını istemediğimizden
        ...
        for (Role w :roles) {
            authorities.add( new SimpleGrantedAuthority(w.getName().name()));// role.getName().name() yazmamızın nedeni Enums dan gelen yapının ROLE_STUDENT şeklinde gelmesini istememiz
        }
        return authorities;
    }
}
